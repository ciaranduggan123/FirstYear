...
struct dateAndTime //structure for date and time
{
    int day;
    int hour;
    int minute;
...
struct ProductionLog //structure for each production logs
{
    int lineCode;
    int batchCode;
...
void merge(struct ProductionLog arr[], int left, int middle, int right) 
{
    ...
    while (i < n1 && j < n2) {
        ...
        else if (L[i].lineCode > R[j].lineCode) 
        {
            arr[k++] = R[j++];
        } 
        else 
...
{
    // Assuming logs are already sorted by date & time
...
int main() 
{
    ...
    // Sort the production logs by date & time
